 ...
 *   계산 결과를 사람에게 알려 주는 상호 작용을 하게 됨.
 * - 이러한 객체는 속성과 기능(동작)으로 구성이 되어 있음.
 *   예를 든다면 사람은 이름, 나이 등의 속성이 있고, 걷다, 쓰다, 달린다
 *   등의 동작이 있음. 또한 자동차의 경우에는 색상, 모델명 등이 속성에
 *   해당이 되고, 달린다, 멈춘다 등의 동작이 있음.
 ...
 * - 상속관계는 상위(부모) 객체를 기반으로 하위(자식) 객체를 생성하는
 *   관계를 말함. 일반적으로 상위 객체는 종류를 의미하고, 하위 객체는
 *   구체적인 사물에 해당이 됨.
 *   예를 든다면 "독수리는 새의 종류이다" 에서 새(상위)와 독수리(하위)는
 *   상속관계에 있다고 볼 수 있음.
 * - 객체지향 프로그래밍(OOP : Object Oriented Programming)은
 *   만들고자 하는 완성품인 객체를 모델링하고, 집합관계에 있는 부품
 *   객체와 사용관계에 있는 객체를 하나씩 설계한 후 조립하는 방식으로
 *   프로그램을 개발하는 기법임.
 */
 ...
 *   만들어짐. 예를 들어 사람들이 자동차를 이용하기 위해서는 우선 
 *   공장에서 설계도를 보고 자동차를 만들어야 함. 객체지향 프로그래밍도
 *   마찬가지임. 메모리에서 사용하고 싶은 객체가 있다면 우선 설계도로
 *   해당하는 객체를 만드는 작업이 필요함.
...
public class Class_01 {
	...
	public static void main(String[] args) { // 멤버 메서드 (메인 메서드도!)
		// 지역변수
		int su = 10;
