 ...
 *    2) 클래스 이름과 동일.
 *    3) 클래스에 포함되어 있으며, 객체를 생성할 때 자동으로
 *       딱 한 번만 호출이 됨.
 *    4) 클래스는 생성자를 통해서 객체로 생성이 됨.
 *       생성자 형식)
 *       			[접근제한] 생성자이름(매개변수) { //매개변수가 있을 수 도 있고 없을 수 도
 *       				생성자 호출 시 실행될 문장;
 *       			}
 ...
 *    7) 기본 생성자 외에 다른 생성자를 만들면, 숨어 있던 기본 생성자는
 *       사라짐.
 *    8) 기본 생성자 외에 다른 생성자를 만들면, 무조건 기본 생성자를 
 *       만들어 줄 것.
...
public class Student {
	...
	public Student() { // 기본생성자 //클래스를 대상으로 객체를 생성해주는 역할

	} // 기본생성자는 반드시 만들어줘야된다!

	public Student(int h, String n, String m, String p, String a) {
